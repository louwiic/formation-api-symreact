...
class Customer
{
    ...
    /**
     * @ORM\ManyToOne(targetEntity="App\Entity\Utilisateur", inversedBy="customers")
     * @Groups({"customers_read"})
    ...
    /**
     * Permet récupérer le total des invoices
     * @Groups({"customers_read"})
     *
     * @return float
     */
    ...
    /**
     * Récupére le montant total non payé (montant total hors factures payées ou annulées)
     * @Groups({"customers_read"})
     * @return float
     */
    ...
    {
        return array_reduce($this->invoices->toArray(), function ($total, $invoice) {
            return $total + ($invoice->getStatus() === "PAID" or $invoice->getStatus() === "CANCELLED" ? +0 : $invoice->getAmount());
        }, 0);
    }
